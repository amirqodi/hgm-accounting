...
import { ContactTransactions } from "@/components/pages/contacts/ContactTransactions";
...
export default function CustomerDetailPage() {
  ...
  useEffect(() => {
    const fetchCustomer = async () => {
      try {
        ...
        if (!res.ok) {
          notify("error", "خطا در دریافت اطلاعات مشتری");
          return;
        }
  ...
  const onSubmit = async (values: CustomerUpdateFormData) => {
      ...
      if (!res.ok) {
        ...
        if (errData) {
          Object.entries(errData).forEach(([field, error]: any) => {
            setError(field as keyof CustomerUpdateFormData, {
              type: "server",
      ...
      >
        ...
        <div className="w-full flex flex-col gap-2 my-2">
          <div className="w-full px-5 flex justify-between items-center gap-3">
            ...
            <input
              type="text"
              value={formatNumber(watch("car_kilometer"))} // تابعی که عدد رو با کاما فرمت می‌کنه
              onChange={(e) => {
                const rawValue = e.target.value.replace(/,/g, ""); // حذف کاما
                setValue("car_kilometer", Number(rawValue), {
                  shouldDirty: true,
                }); // فعال کردن isDirty
              }}
              className="w-[78%] border-background border-2 rounded-lg p-2"
          ...
          {errors.car_type && (
            <p className="text-red-500">{errors.car_kilometer?.message}</p>
          )}
        </div>
