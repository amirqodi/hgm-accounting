...
export default function Page() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setRefreshKey((prev) => prev + 1); // 👈 ری‌لود جدول
    } catch (err: any) {
    ...
    <div className="p-6">
      ...
      <DataTable<any>
        ...
        actions={[
          {
            label: "مشاهده",
            onClick: (row) => router.push(`/contacts/vendors/${row.id}`),
            className: "text-blue-500 cursor-pointer",
          },
          {
            label: "ویرایش",
            onClick: (row) =>
              router.push(`/contacts/vendors/${row.id}?edit=true`),
        ...
        title="افزودن مشتری"
        href="/contacts/add"
        apiUrl={`${process.env.NEXT_PUBLIC_API_URL}/api/contacts/?type=vendor&`}
        page={page}
        search={search}
